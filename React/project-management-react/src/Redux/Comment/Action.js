import api from "@/config/api";
import * as actionTypes from "./ActionTypes";

export const createComment = (commentData) => {
    return async (dispatch) => {
        dispatch({ type: actionTypes.CREATE_COMMENT_REQUEST });
        try {
            const response = await api.post(
                `/api/comments`,
                commentData
            );
            console.log("comment created ", response.data)
            dispatch({
                type: actionTypes.CREATE_COMMENT_SUCCESS,
                comment: response.data, // This adds the new comment
            });
            dispatch(fetchComments(commentData.issueId)); // Fetch updated comments after creating a new one
        } catch (error) {
            console.log("error ", error)
            dispatch({
                type: actionTypes.CREATE_COMMENT_FAILURE,
                error: error.message,
            });
        }
    };
};


export const deleteComment = (commentId) => {
    return async (dispatch) => {
        dispatch({ type: actionTypes.DELETE_COMMENT_REQUEST });
        try {
            await api.delete(
                `/api/comments/${commentId}`,
            );
            dispatch({ type: actionTypes.DELETE_COMMENT_SUCCESS, commentId });
        } catch (error) {
            console.log("error ", error)
            dispatch({
                type: actionTypes.DELETE_COMMENT_FAILURE,
                error: error.message,
            });
        }
    };
};

export const fetchComments = (issueId) => {
    return async (dispatch) => {
        dispatch({ type: actionTypes.FETCH_COMMENT_REQUEST });
        try {
            const response = await api.get(
                `/api/comments/${issueId}`
            );
            dispatch({
                type: actionTypes.FETCH_COMMENT_SUCCESS, // Correct action type
                comments: response.data, // Fetch comments successfully
            });
            console.log("fetched comments ", response.data)
        } catch (error) {
            console.log("error ", error)
            dispatch({
                type: actionTypes.FETCH_COMMENT_FAILURE,
                error: error.message,
            })
        }
    };
};
